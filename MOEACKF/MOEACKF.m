...
% Multi-objective evolutionary algorithm based on cross-scale knowledge
% Fusion

%------------------------------- Reference --------------------------------
% Z. Ding, L. Chen, D. Sun, X. Zhang, Efficient Sparse Large-Scale 
% Multi-Objective Optimization Based on Cross-Scale Knowledge Fusion, 
% IEEE Transactions on XXXX, 2022.
%------------------------------- Copyright --------------------------------
    ...
    methods
        function main(Algorithm,Problem)
            ...
            while Algorithm.NotTerminated(Population)  
                ...
                [Local_Knowlege,Global_Knowledge,Fitness,NSV,SV,theta]= SparsityAnalysis(Population,Mask,FrontNo,Fitness,GROUP);
                GROUP=[NSV;SV];
                % dual dimension reduction
               if ~isempty(Pop1)
                   [OffDec1,OffMask1,len1]=Reproduction1(Pop1,Dec1,Mask1,FrontNo,CrowdDis,Pop1_Site,Local_Knowlege,Global_Knowledge,NSV,SV,theta,REAL);
               ...
               if ~isempty(Pop2)
                   ...
                   [OffDec2,OffMask2] = Reproduction2(Dec2(MatingPool,:),Mask2(MatingPool,:),SV,NSV,REAL);
                   Offspring2 = SOLUTION(OffDec2.*OffMask2);
               else
