function [Population,Dec,Mask,sRatio] = EnvironmentalSelection(Population,Dec,Mask,N,len,num,V)
...
%% Delete duplicated solutions
    ...
    [~,uni] = unique(Population.objs,'rows'); %筛除向量中的重复值，产生的结果按升序排列
    if length(uni) == 1
    ...
    Dec        = Dec(uni,:);
    Mask       = Mask(uni,:);
    N          = min(N,length(Population)); %N是进化到下一种群中的N 
    ...
    s1     = sum(success(len+1:len+num));%成功进入到下一代的个体
