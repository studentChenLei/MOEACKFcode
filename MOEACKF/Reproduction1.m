function [OffDec,OffMask,len1]=Reproduction1(Pop1,Dec1,Mask1,FrontNo,CrowdDis,Pop1_Site,Local_Knowlege,Global_Knowlege,NSV,SV,theta,REAL)
    len1=length(Pop1);
...
%             allZero=Global_Knowlege(2,:);
    ...
    if REAL
        ...
        T_OffDec=PM(T_OffDec,~allZero);
        %  to reduce the computational complexity of PCA
        if sum(allZero)>0
            OffDec=zeros(N,D);
            OffDec(:,allZero)=GAhalf(Dec1(MatingPool,allZero),allZero);
...
function Offspring=PM(Offspring,site)
...
%         Problem = PROBLEM.Current();
    ...
    function Offspring=GAhalf(Parent,allZero)
